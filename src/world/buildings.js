...
const KINDS = {
  ...
  // A cluster of geodesic domes — the pack's biodome stood in for the Amazon Seattle
  // Spheres. Three domes of stepped size sitting together read as the campus glasshouses,
  // and the accent recolours their trim to the zone's own colour.
  spheres(c, rand) {
    c.add('basemodule_E', { x: 0, z: 0 })
    c.add('basemodule_E', { x: 1.35, z: 0.55, s: 0.78, ry: rand() * 6.28 })
    c.add('basemodule_E', { x: -1.2, z: 0.65, s: 0.62, ry: rand() * 6.28 })
    if (rand() > 0.5) c.add('lights', { x: 0.2, z: -1.3, s: 0.85 })
    return 'Spheres'
  },
...
export function createBuilding({ seed = 1, accent = 0xc96442, kind = null } = {}) {
  ...
  // Lean the colony toward a corporate campus look: about a third of sites become the glass
  // Spheres or a tower, the rest still draw from the full catalogue for variety. A forced
  // `kind` always wins.
  const CORPORATE = ['spheres', 'spheres', 'tower']
  const chosen = kind && KINDS[kind]
    ? kind
    : rand() < 0.34
      ? CORPORATE[Math.floor(rand() * CORPORATE.length)]
      : KIND_IDS[Math.floor(rand() * KIND_IDS.length)]
