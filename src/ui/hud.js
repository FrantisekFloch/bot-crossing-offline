...
const ICON = {
  ...
  parking: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="3"/><path d="M9 17V7h3.2a2.6 2.6 0 0 1 0 5.2H9" stroke-linecap="round"/></svg>`,
...
export class Hud {
  ...
  _wire() {
    ...
    on('#btn-park-project', 'click', () => this.actions.parkProject?.())
  ...
  /**
   * OFFLINE BUILD: a centered, dismissible "locked" panel shown when someone tries to open a
   * demo bot. Stays until dismissed (click backdrop, press the button, or Escape).
   */
  lockedMessage({ title = 'Access locked', body = '', tool = '' } = {}) {
    this.$('.locked-modal')?.remove()
    const overlay = document.createElement('div')
    overlay.className = 'locked-modal'
    const sheet = document.createElement('div')
    sheet.className = 'sheet panel'
    const h = document.createElement('h2')
    h.textContent = tool ? `${title} · ${tool}` : title
    const p = document.createElement('p')
    p.textContent = body
    const btn = document.createElement('button')
    btn.className = 'btn primary'
    btn.textContent = 'Got it'
    sheet.append(h, p, btn)
  ...
  /** Reflect orbit mode on the rail button, if it is present (the rail may be removed). */
  setOrbit(on) {
  ...
  toggleUi(force) {
    ...
    // The hide button was removed from the main screen; update it only if present.
    const hideBtn = this.$('#btn-hide')
    if (hideBtn) hideBtn.innerHTML = this.visible ? ICON.eye : ICON.eyeOff
  ...
  <div class="side-body">
    ...
    <div class="project-detail">
      ...
      <div class="project-actions">
        ...
        <div class="pair">
          <button class="btn" id="btn-park-project" title="Park this repo — move it to the silver lot, still on the map">${ICON.parking} Park</button>
          <button class="btn" id="btn-hide-project" title="Hide this repo from the colony — does not archive its threads">${ICON.eyeOff} Hide</button>
        </div>
      </div>
