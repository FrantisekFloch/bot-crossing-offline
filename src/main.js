...
import { hideProject, hiddenCatalog, unhideProject, parkProject, unparkProject } from './game/hidden-projects.js'
...
// How often the colony re-scans the harnesses. Lengthened from 15s — the map does not need
// to react second-to-second, and a longer interval is easier on the machine. A tab regaining
// focus still triggers an immediate catch-up poll (see the focus listener below).
const POLL_MS = 60000
...
let state = { archived: [], archivedAt: {}, opened: [], plots: {}, seen: {}, hiddenProjects: [], parkedProjects: [], viewedAt: {} }
let threads = []
...
const actions = {
  ...
  /** Park the open repo — it moves to the silver parking lot rather than leaving the map. */
  parkProject: () => {
    const name = selectedProject
    if (!name) return
    state.parkedProjects = parkProject(state.parkedProjects || [], name)
    queueSave()
    applyThreads(threads)
    hud.toast(`Parked — moved to the lot, still on the map`)
  },

  unparkProject: (name) => {
    const target = name || selectedProject
    if (!target) return
    state.parkedProjects = unparkProject(state.parkedProjects || [], target)
    queueSave()
    applyThreads(threads)
    hud.toast(`Unparked — back among the active zones`)
  },
  ...
  openThread: async () => {
    const thread = threads.find((t) => t.id === selectedId)
    if (!thread) return
    // OFFLINE BUILD: these are demo bots, not real sessions on this machine. Opening one shows
    // a friendly "locked" message instead of handing a ref back to a harness.
    hud.lockedMessage({
      title: 'Access locked',
      tool: thread.harnessName || '',
      body:
        "Hi there — the API key I'm reading says you're not František, so I can't unlock " +
        'his models for you. This is a demo colony: the bots are real, the access is not. ' +
        'Want to see the magic for yourself? Reach out to František to work with him.',
    })
  },
...
function applyThreads(list) {
  ...
  const parkedSet = new Set(state.parkedProjects || [])
  ...
  const stats = colony.setThreads(list, archivedSet, hiddenSet, known, parkedSet)
  hud.setStats(stats)
