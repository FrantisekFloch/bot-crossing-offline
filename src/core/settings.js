...
// Bumped v1 → v2 so existing browsers adopt the new crisp defaults (higher render scale,
// SMAA on, tilt-shift off) instead of silently keeping their old stored "balanced" values.
const STORE_KEY = 'botcrossing.settings.v2'
...
export const PRESETS = {
  ...
  balanced: {
    label: 'Balanced',
    hint: 'the default — crisp, runs cool',
    values: {
      // Supersample a little above native for crisper buildings and outlines.
      renderScale: 1.25,
      shadows: 'high',
      bloom: true,
      // Crisp by default: SMAA on and the tilt-shift depth-of-field blur off, so the
      // colony reads sharp edge to edge rather than soft toward the frame edges.
      antialias: true,
      particles: 'low',
...
const DEFAULTS = {
   ...
   */
  hideDormant: true,
  // Default view opens at 13:40 on Luna, just past the noon sun. 0..1 where 0.5 = 12:00,
  // so 13:40 = (13 + 40/60) / 24 ≈ 0.5694. autoTime/clockTime off so it holds at 13:40.
  timeOfDay: 0.5694, // 13:40
  autoTime: false,
  ...
  // Behaviour
  autoQuality: false, // stay at native resolution; the governor could otherwise render sub-native and read as soft
