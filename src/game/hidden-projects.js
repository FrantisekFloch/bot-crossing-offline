...
/**
 * Repos you have **parked**. Unlike hiding, a parked repo stays on the map — it is just moved
 * out to the silver parking lot around the "P" sign, the way a long-dormant one is. It is for
 * a repo you want out of the busy middle without forgetting it exists. Same name-keying and
 * same trade-off as hiding.
 */
export function parkProject(parked, name) {
  const id = String(name || '')
  if (!id || parked.includes(id)) return [...parked]
  return [...parked, id]
}

export function unparkProject(parked, name) {
  const id = String(name || '')
  return parked.filter((n) => n !== id)
}
