...
// A zone is "dormant" once nothing in it has stirred for 30 days. Dormant zones are parked
// off to one side (see the parking cluster) rather than folded off the map.
...
/** Neutral silver for parked (dormant) zones, so they read as "set aside" at a glance. */
const DORMANT_ACCENT = 0x9aa0a6

/** Cut a label to `max` characters with an ellipsis, so parked zones' names fit their plot. */
...
export class Colony {
   ...
   */
  setThreads(threads, archivedIds = new Set(), hiddenProjects = new Set(), knownIds = new Set(), parkedProjects = new Set()) {
    const now = Date.now()
    const live = liveThreadsForColony(threads, archivedIds, hiddenProjects)
    const parked = parkedProjects instanceof Set ? parkedProjects : new Set(parkedProjects)
     ...
     */
    // A zone goes to the silver parking lot if EITHER you parked it by hand OR everything in
    // it has been quiet 30+ days. Parked/dormant zones are not folded off the map — they are
    // moved out to the lot around the "P" sign. They stay in `byProject`.
    const dormant = new Set()
    for (const [name, list] of byProject) {
      const auto = list.every((t) => statusFor(t, now) === 'sleeping')
      if (auto || parked.has(name)) dormant.add(name)
    }
    this.dormantProjects = dormant

    // The parking lot is marked with a big blue "P". Drawn only while something is parked.
    this._syncMarkers(dormant.size > 0)

    // Live zones first and biggest-first (they seed nearest the hub); dormant zones sort
    // among themselves for the parking lot. Each entry carries its region so the allocator
    // and the plot styling know where it belongs.
    const projects = [...byProject.entries()]
      .map(([name, list]) => [name, list, dormant.has(name) ? 'parking' : 'live'])
      .sort((a, b) => {
        // Live before parking, then biggest first, then by name for stability.
        if ((a[2] === 'parking') !== (b[2] === 'parking')) return a[2] === 'parking' ? 1 : -1
    ...
    // A hidden repo keeps its footprint in layout memory, so showing it again reclaims the
    // same ground if it is still free.
  ...
  /**
   * The parking-lot marker: a blue "P" in a white triangle (the road parking sign) standing
   * in the open centre of the parking cluster, with parked zones ringed around it. Built
   * once and shown only while something is parked. No hub marker any more — the live colony
   * fills its own centre.
   */
  _syncMarkers(showParking) {
    if (!this._markers) this._markers = { parking: null }

    if (showParking && !this._markers.parking) {
      const sign = createParkingSign()
      const { x, z } = parkingWorld()
      sign.position.set(x, 4.0, z)
      sign.scale.setScalar(2.0) // a landmark for the whole lot
      sign.userData.persistent = true
      this._markers.parking = sign
      this.labelGroup.add(sign)
    } else if (!showParking && this._markers.parking) {
      this.labelGroup.remove(this._markers.parking)
      this._markers.parking.userData.dispose?.()
      this._markers.parking = null
    }
  }
  ...
  _syncPlots(projects) {
    ...
    const regionFor = new Map(projects.map(([name, , region]) => [name, region || 'live']))
    const layout = allocateCells(
      projects.map(([name, list, region]) => ({ id: name, size: list.length, region: region || 'live' })),
      this.plotCells
    )
    ...
    // The clean name shown on each zone's plate: an adapter can supply `projectLabel`
    // (Kiro does — the session title); otherwise the project key doubles as the label.
    // Parked zones sit close together in the lot, so their names are cut short with an
    // ellipsis to fit their small footprint.
    const labelFor = new Map()
    // Which tool each zone's work runs in, so the plate can show that tool's icon instead of
    // a plain accent dot.
    const harnessFor = new Map()
    for (const [name, list, region] of projects) {
      const full = list[0]?.projectLabel || name
      labelFor.set(name, region === 'parking' ? truncateLabel(full, 14) : full)
      harnessFor.set(name, list[0]?.harness || '')
    }

    const wanted = new Map()
    // The label and region are folded into the signature so a zone rebuilds if its title
    // changes or it moves between the live and parking clusters, not only when cells move.
    for (const [name, cells] of layout) {
      const sig = `${name}:${labelFor.get(name) || ''}:${regionFor.get(name)}:${cells.map((c) => `${c.q},${c.r}`).join('/')}`
      wanted.set(name, sig)
    }
    ...
    projects.forEach(([name], index) => {
      ...
      // Dormant zones in the parking lot are drawn in a neutral silver so they read as
      // "put aside" at a glance; live zones keep their own Amazon accent.
      const accent = regionFor.get(name) === 'parking' ? DORMANT_ACCENT : this._pickAccent(name)
      const displayName = labelFor.get(name) || name
      const plot = new Plot({ id: name, name: displayName, index, cells, accent })
      plot.signature = wanted.get(name)
      this.plots.set(name, plot)
      this.plotGroup.add(plot.group)

      const label = createLabel(displayName, accent, harnessFor.get(name))
  ...
  _updateLabels(dt) {
    ...
    for (const plot of this.plotOrder) {
      const label = plot.label
      if (!label) continue
      // Every zone keeps its name on screen so the map is readable at a glance. Active or
      // hovered zones show at full strength; the rest sit at a quieter 0.6 so the busy ones
      // still stand out. (Original behaviour only showed active/hovered names.)
      const lit = this.activePlots.has(plot.id) || this.hoveredPlot === plot
      const wanted = show ? (lit ? 1 : 0.6) : 0
      const next = THREE.MathUtils.damp(label.material.opacity, wanted, 9, dt)
      label.material.opacity = next
      label.visible = next > 0.01
    }
    // The parking "P" sign is always on while the UI is up — it anchors the parking lot.
    const sign = this._markers?.parking
    if (sign) {
      const wanted = show ? 1 : 0
      const next = THREE.MathUtils.damp(sign.material.opacity, wanted, 9, dt)
      sign.material.opacity = next
