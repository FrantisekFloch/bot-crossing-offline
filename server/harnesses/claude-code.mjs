...
/**
 * Each Claude session becomes its own zone, the same way Kiro does — one plot per conversation
 * rather than one crowded plot per folder. `zoneKey` is the unique grouping key (never shown);
 * `zoneLabel` is the clean title on the plate.
 */
const zoneKey = (rawId) => `claude-code:${rawId}`
const zoneLabel = (title) => String(title || '').replace(/\s+/g, ' ').slice(0, 40).trim() || 'Untitled'
...
async function scanThreads() {
  ...
  for (const s of desktop) {
    ...
    const { projectPath, project: folderName, worktree } = projectOf(cwd, s.originCwd)
    const meta = entry ? await transcriptMeta(entry) : null
    const rawId = cliSessionId || s.sessionId
    ...
    add({
      ...
      // One zone per session (see zoneKey): a unique key, with the clean title on the plate
      // and the workspace folder kept for reference.
      project: zoneKey(rawId),
      projectLabel: zoneLabel(title),
      folder: folderName,
      projectPath,
      worktree,
      cwd,
  ...
  for (const [id, entry] of transcripts) {
    ...
    const { projectPath, project: folderName, worktree } = projectOf(cwd, '')
